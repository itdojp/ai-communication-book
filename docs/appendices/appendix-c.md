...
AIエージェントの出力を実務で活用する際は、「会話ログ」だけで終わらせず、**成果物（意思決定・変更管理・運用手順・障害記録）**に落とし込むことが重要です。本付録では、そのためのtemplateをまとめます。
...
- [ ] 事実、仮定、判断、未確認点が区別されている
- [ ] owner、reviewer、approverが明確である
- [ ] 根拠・version・確認日を追跡できる
- [ ] 高risk操作に承認ゲートと停止条件がある
- [ ] testまたは実地確認の証拠がある
- [ ] rollbackまたはfailure recoveryを定義した
- [ ] AI利用範囲と人間が確認した範囲を説明できる
