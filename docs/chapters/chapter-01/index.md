...
> 本書では、AIエージェントに依頼する際の共通運用として [AIエージェント協働の標準手順（SOP）](../../introduction/agent-protocol/) を定義している。特に「承認ゲート」「検証」「責任分界」は、後続章（RAG/Function Calling・品質/リスク管理）でも同じ前提で参照する。

> **この章で学ぶこと**
> - 5つの基本テンプレート（情報抽出・要約・創作支援・分析支援・問題解決）を使って、日常業務の1つをAIに任せる体験をすること
> - CLEAR手法などの基本的なプロンプト設計の考え方を押さえ、「なんとなく聞く」から「意図して指示する」に切り替えること
...
> 注: 本章の図表・数値例は説明のための目安であり、効果はタスク/組織/評価基準により変動する。
...
│                   STEP 4：品質検証                             │
│                                                                │
│ [OK] 明確性：指示が具体的で理解しやすいか                         │
│ [OK] 完全性：必要な情報がすべて含まれているか                     │
│ [OK] 一貫性：矛盾する要求が含まれていないか                       │
│ [OK] 実現性：AIが対応可能な範囲内か                               │
└─────────────────────┬───────────────────────────────────────────┘
                      │
                      ▼
...
> 注: 本節の測定例の数値・期間は例示であり、実務では自組織のKPIに合わせて設計・再計測する。
...
- 生成AIとのコミュニケーションを「即効性のある活用法」として整理し、5つの基本テンプレートと CLEAR手法など、すぐに試せるパターンを提示した。
...
- [ ] CLEAR手法の各要素（Context / Length / Examples / Audience / Role）を意識したプロンプトを、自分なりに 1 本以上書いてみたか。
...
- 「会話で終わらせず成果物まで落とす」実務の流れを掴むには、[付録D：実務会話例集（成果物まで落とす）](../../appendices/appendix-d/) の例を参照するとよい。
