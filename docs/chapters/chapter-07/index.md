...
技術的な優秀性だけでなく、組織的な受容性や運用継続性、投資対効果も総合的に考慮する。実践的なアプローチを提示する。
...
> 注: 以降の金額・工数・割合は説明のためのモデルケースである。自組織で扱う際は「測定期間」「前提条件」「算出方法」「検証手順」をセットで明記し、意思決定ログとして残すことを推奨する。
...
評価基準：
※ 目標値（80%/90%など）は例であり、タスク定義・測定方法・リスク許容度に合わせて設定する
- 基本操作：エラーなく実行可能
...
- ADR/PR/Runbook/ポストモーテムといった成果物に落とす際の雛形は、[付録C：成果物テンプレート集（ADR/PR/Runbook/ポストモーテム）](../../appendices/appendix-c/) を参照してほしい。
- 実務での会話の流れ（確認質問→Plan→承認→検証→報告）をまとめて確認するには、[付録D：実務会話例集（成果物まで落とす）](../../appendices/appendix-d/) が役立つ。
