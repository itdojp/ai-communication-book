...
効果的なAI活用のためには、AIがどのように動作するかを理解することが重要である。あわせて、何ができて何ができないのかを把握しておく必要がある。本章では、生成AIの核心技術であるトランスフォーマーアーキテクチャから、確率的生成の特性、スケーリング法則まで、実用的な活用につながる技術的本質を解説する。

複雑な数学的詳細は省略し、AIとの効果的なコミュニケーションに必要な「技術的直感」の獲得を目指す。

> **注意（更新されやすい情報）**  
> 本章では理解を助けるため、モデル名・コンテキスト長・料金などの具体例を一部挙げる。これらは執筆時点の参考であり、最新仕様・最新料金は各社の公式情報で要確認である。

> **この章で学ぶこと**
> - トランスフォーマーをはじめとする現代的な生成AIのアーキテクチャが、従来手法とどのように異なるかを直感的に説明できるようになること。
> - 確率的生成やスケーリング法則といった性質が、AIの得意／不得意や出力のブレにどのようにつながるのかを理解し、期待値設定やプロンプト設計に反映できるようになること。
> - 「どのようなタスクを AI に任せ、どのような判断は人間が担うべきか」という分担の観点を、技術的背景に基づいて考えられるようになること。
...
問題：
- 1回の処理で扱える情報量に限界があり、全体を一度に扱いきれない場合がある
- 後半部分の処理で前半の情報が参照されにくく、整合性が崩れることがある
- 文書間の関連性の把握が困難
...
> 注: パラメータ数は公式に開示されない場合があり、推定値を含む。参考値として扱い、実務の判断は自組織の評価結果を優先する。
...
※料金はモデル/プラン/契約条件で変動するため、最新仕様は各社公式情報で要確認
```
...
> 注: 将来予測は不確実性が高い。計画に用いる場合は根拠と前提を明記し、定期的に見直す。
...
次に読む： [第3章：モデル評価と選択基準](../chapter-03/) / [目次（トップ）](../../)
...
- トランスフォーマーアーキテクチャ、トークン化とコンテキスト制約、確率的生成、スケーリング法則といった技術的要素を整理し、生成AIの振る舞いを理解するための直感的な枠組みを提示した。
- コンテキスト長制限や確率的生成の性質が、プロンプト設計やコンテキスト制御にどのような影響を与えるかを示した。
- 技術詳細の暗記ではなく、「AI が得意／苦手とする領域を踏まえた活用戦略」を立てるための土台となる技術理解の重要性を強調した。

### この章を読み終えたら確認したいこと

- [ ] トランスフォーマーと従来モデル（RNN 等）の違いを、非専門家にも説明できるレベルで整理できるか。
- [ ] コンテキスト長やトークン化の制約が、自分の想定するユースケースにどのような制限を与えるかを説明できるか。
- [ ] 確率的生成の性質を踏まえ、「どのタスクでは多様性を上げたいか／下げたいか」を具体的に考えられるか。

### 関連する付録・テンプレート

- 本章で登場した技術用語や論文の詳細は、[付録B：参考文献](../../appendices/appendix-b/) を参照しながら、興味のあるテーマを深掘りしてほしい。
