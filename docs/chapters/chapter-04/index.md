...
効果的なプロンプト設計は、AIに対する「仕様書」の作成に類似している。明確で論理的な構造により、期待する結果に近づけ、再現性を高められる。
...
複雑な問題に対してAIから高品質な回答を得るには、人間の問題解決プロセスをなぞる段階的アプローチが有効である。

> 注記: 実務では「思考過程の逐語的な開示」を求めるのではなく、各ステップの**中間成果物**（観点、仮説、チェックリスト、計画など）を箇条書きで出力させることを推奨する。これにより、検証とレビューに必要な材料を確保しやすい。
...
> 注記: ここでも「思考過程の逐語的な開示」ではなく、複数案の列挙→評価→統合という**中間成果物**を出力させる（例: 案ごとの比較表、採否理由、検証計画）形にすると、業務で扱いやすい。
...
次に読む： [第5章：高度プロンプト技術](../chapter-05/) / [目次（トップ）](../../)
