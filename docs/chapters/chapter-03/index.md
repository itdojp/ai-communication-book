...
> 注: ベンチマーク結果は出題形式や評価条件（プロンプト/温度など）で変動する。実務では自組織のタスクで評価手順を作り、再現可能な条件で比較する。
...
具体的なモデル名やスコアは変化が速いため、本文では固定値を列挙しすぎず、評価軸と比較手順をテンプレート化して使う方が保守性が高い。公式情報の確認手順は [付録E：用語集と更新確認ノート](../../appendices/appendix-e/) にまとめた。

```text
【評価軸の組み合わせ例（テンプレート）】
                    知識理解  常識推論  専門性  言語品質  コスト  備考
                    (MMLU)   (HellaSwag) (HumanEval) (日本語)
候補A（高性能重視）   高        高        中〜高     高      高     重要文書や高精度用途向け
候補B（均衡型）       中〜高    中〜高    中        高      中     顧客対応や標準業務向け
候補C（低コスト重視） 中        中        中        中      低     大量処理や下書き用途向け

→用途別の見方：
- 技術文書作成：専門性、根拠提示、再現性を重視する
- 顧客対応：言語品質、常識推論、ガードレール運用を重視する
- 一般業務：コスト、レイテンシ、レビュー工数の総量で判断する
```
...
【料金比較の観点（実単価は公式情報を参照）】
（注）
- 料金はモデル/プラン/契約条件で変動するため、最新仕様は各社公式の料金ページで確認する
- 比較時は「入力単価」「出力単価」「従量課金の単位」「最低利用量」「管理機能の追加費用」を分けて記録する

高性能帯：
- 特徴：高精度、高コスト、重要文書や複雑推論に向く

均衡帯：
- 特徴：品質とコストのバランスが良く、一般業務に向く

低コスト帯：
- 特徴：下書きや大量処理向け。レビュー前提で使う
```

**業務別コスト効率分析**

```text
【月間1000件の技術文書作成の場合（例示）】
...
月間コスト計算式：
- 月間コスト = ((入力トークン数 ÷ 課金単位) × 入力単価 + (出力トークン数 ÷ 課金単位) × 出力単価) × 件数

比較時に見る項目：
- 月間コスト総額
- 期待品質（再現率、レビュー差戻し率、誤回答率など）
- レビュー工数
- 事故時の影響額

→一見単価が低いモデルでも、レビュー工数や誤回答対応コストが大きければ総コストは逆転する。逆に高性能モデルでも、重要度の低い用途では過剰品質になることがある。
```
...
例（モデルケース）：
（※以下の金額は仮の数値例であり、実際の料金体系・モデル・地域・契約条件とは無関係なサンプルです）
現状：均衡型モデル（精度80%、コスト$5）
改善案：高性能モデル（精度90%、コスト$65）
...
- モデル名・料金・リリース情報の確認手順は、[付録E：用語集と更新確認ノート](../../appendices/appendix-e/) を参照してほしい。
