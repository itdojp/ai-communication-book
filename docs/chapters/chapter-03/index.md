...
```text
【料金比較（例示。実際の単価は公式情報を参照）】
（注）
- 料金はモデル/プラン/契約条件で変動するため、最新仕様は各社公式の料金ページで確認する
- 以下はコスト計算の考え方を示すための例示であり、実単価を保証しない
...
- GPT-4：品質単位あたり$14.4
- GPT-3.5：品質単位あたり$1.3
→このモデルケースではGPT-3.5が効率的に見える（品質要件/リスク/検証コストにより選択は変わり得る）
```
...
Tier 2（重要度：中）：
- 対象：内部資料、標準的な分析
- モデル：中性能モデル（例: Claude 3 / GPT-3.5）
- バランス：品質とコストの適度なバランス

Tier 3（重要度：低）：
- 対象：下書き、大量処理、実験的用途
- モデル：低コストモデル（例: Gemini Pro / GPT-3.5）
- 重視：コスト効率の最大化
