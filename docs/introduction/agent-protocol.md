...
## 依頼契約ゲート（Request Contract）

AIエージェントとの対話を実務に使う場合、依頼は「お願い」ではなく、成果物に対する契約として扱う。最低限、次の6項目を揃える。

| 項目 | 確認すること | 成果物に残す例 |
| --- | --- | --- |
| Intent | 何を達成するか、誰のためか | 目的、対象読者、利用シーン |
| Context Boundary | 何を入力し、何を入力しないか | 参照資料、機密区分、外部送信禁止情報 |
| Output Contract | どの形式・粒度で返すか | 表、箇条書き、ADR、PR案、Runbook |
| Acceptance Check | 何を満たせば採用できるか | 受け入れ条件、レビュー観点、失敗条件 |
| Verification | 何で裏取りするか | 一次情報、テスト、再現手順、人間レビュー |
| Owner | 誰が採否とリスクを引き受けるか | 判断責任者、承認者、エスカレーション先 |

このゲートは、AIの回答品質を高めるだけでなく、後続のレビュー、監査、改善を可能にするためのものである。依頼契約が曖昧なまま実行すると、出力は一見よく見えても、採用条件や責任分界を説明できない。
